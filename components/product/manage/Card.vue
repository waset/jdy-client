<script setup lang="ts">
</script>

<template>
  <div class="grid-12">
    <div class="col-12 lg:col-8 lg:offset-2 grid-12 gap-[24px]">
      <template v-for="(item, index) in 6" :key="index">
        <div
          class="col-12 sm:col-6 rounded-[24px] bg-gradient-linear-[90deg,#EDF3FE,#E3EEFD] dark:bg-gradient-linear-[90deg,#23324B,#2F548C] p-[1px]">
          <div
            class="py-[8px] px-[16px] bg-gradient-linear-[90deg,#E9F1FE,#95D5FB] dark:bg-gradient-linear-[90deg,#23324B,#70B8E8] rounded-t-[24px] flex-start text-black dark:text-[#FFF]">
            <common-avatar :wh="32" />
            <div class="ml-[8px] font-semibold line-height-[20px] text-size-[14px]">
              标题/名称/单号/条码示例
            </div>
          </div>
          <div class="px-[16px] py-[8px] text-size-[14px] line-height-[20px] text-black dark:text-[#FFF]">
            <van-row justify="space-between" class="py-[4px]">
              <van-col span="12">
                <div class="">
                  所属大类
                </div>
              </van-col>
              <van-col span="12">
                <div class="text-align-end">
                  12313
                </div>
              </van-col>
            </van-row>
            <van-row justify="space-between" class="py-[4px]">
              <van-col span="12">
                <div class="">
                  所属大类
                </div>
              </van-col>
              <van-col span="12">
                <div class="text-align-end">
                  12313
                </div>
              </van-col>
            </van-row>
          </div>
          <div class="bg-[#F3F5FE] dark:bg-[#F3F5FE1A] rounded-b-[24px]">
            <van-row justify="space-between">
              <van-col span="8">
                <div class="py-[8px] px-[16px]">
                  <span
                    class="bg-[#FFF4CD] px-[8px] text-size-[14px] line-height-5 color-[#DD9200] rounded-[8px] text-center">
                    调拨在途
                  </span>
                </div>
              </van-col>
              <van-col span="12">
                <div class="flex-end text-size-[14px]">
                  <div class="mr-[16px] line-height-[20px] text-[#3971F3] dark:text-[#FFF]">
                    更多
                  </div>
                  <div
                    class="bg-gradient-linear-[180deg,#1F6FEC,#68A2FE] px-[41px] py-[8px] rounded-br-[24px] line-height-[20px] text-[#FFF]">
                    编辑
                  </div>
                </div>
              </van-col>
            </van-row>
          </div>
        </div>
      </template>
    </div>
  </div>
</template>
